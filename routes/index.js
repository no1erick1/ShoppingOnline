...
router.get('/', function(req, res, next) {  
  ...
  router.get('/chart', function(req, res, next) {  
    ...
    Order.find(function(err, orders){
        ...
        res.render('./categories/viewchart',{orders: orders , successMsg: successMsg, noMessage: !successMsg});
    });
  });
 ...
 /*  router.get('/categories/vieworder',function(req,res,next){
    Order.find(function(err, docs){
     ...
     order.items = cart.generateArray();
       ...
       res.render('categories/vieworder',{orders: orders, productChunks});
    });
  });   */

  router.get('/categories/vieworder',isLoggedIn, function(req,res,next){
    var successMsg = req.flash('success')[0];
      Order.find(function(err, orders){
          if(err){
            return res.write('Error!');
          ...
          res.render('./categories/vieworder',{orders: orders , successMsg: successMsg, noMessage: !successMsg});
      });
  });
...
router.post('/shop/updatepro', (req,res,next)=>{
  ...
  var proId = Allproduct.findOne({_id:proId}, (err, pro)=>{
    ...
    pro.save((err, pro)=>{
      ...
      res.redirect("/shop/updatepro?id="+pro.id)
    })
  })
});
...
router.post('/shop/updateuser', (req,res,next)=>{
  ...
  var user = User.findOne({_id:userId}, (err, user)=>{
    ...
    user.save((err, user)=>{
      ...
      res.redirect("/shop/updateuser?id="+user.id)
    })
  })
});
...
//chart
router.get('/categories/viewchart1', function(req, res, next) {  
  var successMsg = req.flash('success')[0];
  Order.find(function(err, orders){
      if(err){
        return res.write('Error!');
      ...
      res.render('./categories/viewchart1',{orders: orders , successMsg: successMsg, noMessage: !successMsg});
  });
});
router.get('/categories/viewchart1',function(req,res,next){
  Order.find(function(err, docs){
   ...
   for(var i=0;i<docs.length;i+=chunkSize){
     ...
     res.render('./categories/viewchart1',{orders: productChunks});
  });
}); 
